...
func init() {
	reader.RegisterReader(&drivers.Driver{Name: "mockdb", Options: driverOpts, CreateReader: newSqlmock, FullName: "mockdb"})
}
...
func newSqlmock() driver_interface.DbReader {
	log.Printf("Connecting to mockdb")
	return mockdbModel{connected: false}
}

func (model mockdbModel) ReadSchema(databaseName string) (database *schema.Database, err error) {
	dbc, mock, err := getConnection()
	if err != nil {
		return
	}
	model.mock = mock
...
func (model mockdbModel) CanSwitchDatabase() bool {
	return false
}

func (model mockdbModel) GetConfiguredDatabaseName() string {
	return ""
}

func (model mockdbModel) ListDatabases() (databaseList []string, err error) {
	panic("not available for sqlite")
}

func (model mockdbModel) DatabaseSelected() bool {
	return true // there is only one
}

func (model mockdbModel) UpdateRowCounts(database *schema.Database) (err error) {
	for _, table := range database.Tables {
...
func (model mockdbModel) getTables(dbc *sql.DB) (tables []*schema.Table, err error) {
	// todo: parameterise
	rows, err := dbc.Query("SELECT name FROM mock_master WHERE type='table' AND name not like 'mock_%' order by name;")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (model mockdbModel) getRowCount(table *schema.Table) (rowCount int, err error) {
	sql := "select count(*) from \"" + table.Name + "\""

	dbc, _, err := getConnection()
	if dbc == nil {
		log.Println(err)
...
func getConnection() (dbc *sql.DB, mock sqlmock.Sqlmock, err error) {
...
func (model mockdbModel) SetDatabase(databaseName string) {
	// n/a
}

func (model mockdbModel) GetDatabaseName() string {
	return "" // n/a
}

func (model mockdbModel) CheckConnection(databaseName string) (err error) {
	...
	if len(tables) == 0 {
		...
		err = errors.New("no tables found. (mock not setup correctly)")
		return
	}
...
func (model mockdbModel) Connected() bool {
...
func getIndexInfo(dbc *sql.DB, index *schema.Index, table *schema.Table) (err error) {
	...
	for rows.Next() {
		...
		if colName != "" {
			...
			if col == nil {
				err = fmt.Errorf("can't find col '%s' specified in index %s", colName, index.String())
				return
			}
...
func (model mockdbModel) GetSqlRows(databaseName string, table *schema.Table, params *params.TableParams, peekFinder *driver_interface.PeekLookup) (rows *sql.Rows, err error) {
	dbc, _, err := getConnection()
	if err != nil {
...
func (model mockdbModel) GetRowCount(databaseName string, table *schema.Table, params *params.TableParams) (rowCount int, err error) {
	dbc, _, err := getConnection()
	if err != nil {
		log.Print("GetRows failed to get connection")
		return
...
func (model mockdbModel) GetAnalysis(databaseName string, table *schema.Table) (analysis []schema.ColumnAnalysis, err error) {
...
func (model mockdbModel) getColumns(dbc *sql.DB, table *schema.Table) (cols []*schema.Column, err error) {
	// todo: parameterise
...
func (model mockdbModel) SetTableDescription(database string, table string, description string) (err error) {
	return
}

func (model mockdbModel) SetColumnDescription(database string, table string, column string, description string) (err error) {
	return
}
